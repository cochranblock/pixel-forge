...
## Act V: The Forge Goes Mobile

### March 20, 2026 — "Get the app store ready"

**GotEmCoach:** Get the pixel forge app store ready.

**KOVA:** Built the APK pipeline. Both Cinder and Quench embedded via `include_bytes!`. 19 MB APK. Touch-friendly egui, device auto-detect, soft keyboard toggle.

**Component:** `android/src/lib.rs` — NativeActivity entry point, model extraction on first launch, keyboard handling
**Impact:** The first Rust-native AI app on Android that generates pixel art offline. 19 MB total. No runtime downloads.

### March 21, 2026 — "The sprites look like ass"

**GotEmCoach:** The sprites look like ass and it hasn't changed much.

**KOVA:** Cinder v1 trained 100 epochs on v1 data. Not enough. Need 300 epochs on curated v2 data with --no-ema for RAM savings. Quench needs its first real training run too.

Training launched across cluster: Cinder v3 on lf, Quench on lf, Cinder 32x32 on gd. All 300 epochs.

### March 22, 2026 — "Do we need the models more efficient for mobile?"

**GotEmCoach:** Do we need the models for APKs to be more efficient for that arch?

**KOVA:** The models are already small. The win is f16 quantization — halve storage, same inference speed. And don't bundle Quench — Cinder-only APK drops to under 10 MB.

**Component:** `quantize.rs` — f32→f16 safetensors conversion, `load_varmap()` casts f16→f32 at load time. Storage savings without compute pain.
**Impact:** Cinder: 4.2→2.1 MB. Quench: 22→11 MB. APK under 10 MB. Android bundles Cinder only, Quench download-on-demand.

### March 22, 2026 — "Make the icon cooler"

**GotEmCoach:** Make the icon cooler for install. Make it dark and match the app aesthetic. Change sparks to blue like cochranblock.

**Component:** `scripts/gen-icon.py` — programmatic icon generation at all mipmap densities. Dark anvil, cyber blue sparks (#00d9ff), PF monogram. App UI recolored to match.
**Impact:** Brand alignment. Pixel Forge looks like it belongs in the cochranblock ecosystem. The icon is generated by code, not by Photoshop — consistent with the project philosophy.
...
|-----------|------|-------|--------|
| TinyUNet (Cinder) | `tiny_unet.rs` | 270 | 4.2 MB model (2.1 f16), mobile-embeddable |
| MediumUNet (Quench) | `medium_unet.rs` | 326 | 22 MB (11 f16), self-attention, encode/decode split |
| AnvilUNet (Anvil) | `anvil_unet.rs` | 341 | 64 MB (32 f16), highest quality tier |
| Training Pipeline | `train.rs` | 770 | RAM-loaded, cosine schedule, CFG, EMA, --no-ema |
| MoE Cascade | `moe.rs` | 201 | First sub-30MB cascading diffusion |
| Expert System | `expert.rs` + `expert_train.rs` | 274 | 4 specialist heads, 804 KB total |
| f16 Quantize | `quantize.rs` | 186 | f32→f16 conversion, transparent load |
...
| Device Detection | `device_cap.rs` | 501 | GPU/RAM probing, tier selection |
| GUI + Android | `app.rs` + `android/` | 860+ | Touch-friendly, Cinder-only APK |
| Discriminator | `discriminator.rs` | 313 | Quality gate with re-roll |
| PoA Signing | `poa.rs` | 293 | Ed25519 Ghost Fabric packets |
| Plugin Protocol | `plugin.rs` | 210 | JSON for kova integration |
| GPU Scheduler | `kova/gpu_sched.rs` | 185 | Lock + queue + VRAM monitoring |
| **Total** | **29 files** | **9,162** | **First MoE diffusion under 30MB** |
...
*Last updated: 2026-03-22 — v0.5.0*
