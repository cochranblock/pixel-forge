...
/// asynchronously from the GPU, applies the inverse z-score on CPU, then
/// clamps + scales to RGB bytes. Matches the desktop `tensor_to_rgba`.
pub async fn finalize_png(
    ...
    normalizer.from_z_pixels(&mut raw, n);
